...
4. Gedik ilerlemesi artık kuşatma ekipmanı tier'i ile kale seviyesi birlikte dikkate alınarak hesaplanır. Birim tanımındaki `siege_breach_max_fort_level` alanı, tier + 2 varsayılanını özel ekipmanlar için yükseltebilir; 1300 senaryosunda Lağımcılar T3 savaş tier'ında kalırken T6 tahkimata kadar gedik açabilir. Uygun ekipman yoksa kuşatma savunucuyu yıpratmaya ve açlık teslimiyetine devam eder, ancak gedik ilerlemez. Kuşatma paneli ve karar penceresi bu yetersizliği hedef ve mevcut kapasiteyle açıkça gösterir.
...
7. Aktif kuşatmaya aynı fraksiyon ya da müttefik fraksiyon destek için normal hareketle girebilir; bu giriş ayrı bir kuşatma başlatmaz ve destek ordusu otomatik olarak başka bir orduyla birleşmez. Destek orduları aktif bölgedeki canlı state'ten her kuşatma tick'inde taranır; sonradan getirilen kuşatma birimleri de `BreachProgress` ve gedik kazanımını güçlendirir. Gedik gücü her kuşatma birimi için `(2 + tier) × CurrentHP/MaxUnitHP × siege_breach_multiplier` olarak toplanır; alanı olmayan eski birimler için katsayı `1.0`dır. Birim sayısı, canlı HP ve veri tanımındaki katsayı arttıkça gedik hızlanır. Varsayılan erişim sınırı `tier + 2`dir: T1 mancınık T3, T2 bombarda T4, T3 top T5 sura kadar etki eder; `siege_breach_max_fort_level` tanımlı özel birimler bu sınırı yükseltebilir. Uyumsuz düşük tier birimler, sayıları fazla olsa bile gedik gücüne katkı vermez. Ondalık ilerleme, geriye uyumlu `BreachProgressRemainder` alanında saklanır. İlgisiz fraksiyonlar yeni kuşatma hamlesi yapamaz.
...
14. Kuşatılan bölgede bölge sahibine veya onun müttefikine ait bir kara ordusu varsa, bu ordu komşu bölgeye çıkmadan önce kuşatan orduya karşı huruç savaşı yapmak zorundadır. Huruç kazanılırsa kuşatma kalkar ve uygun dost/sahipsiz hedefe ilerlenir; kaybedilirse kalan birlikler kuşatılan bölgede kalır ve hareket puanı tükenir. Kayıt yüklenirken kuşatma bölgesi dışında kalan veya yaşamayan `DefenderArmyID` bağlantıları temizlenir; geçerli savunmacı bağlantısı yalnızca bulunduğu bölgenin kuşatmasına uygulanır.
