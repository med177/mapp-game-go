...
Not: Diplomasi hedef listesindeki devlet adı kolonu 340 px ile sınırlandırılır; ilişki/durum kolonu aynı satır geometrisinden daha solda ve kalan genişlikte çizilir. Uzun durum etiketleri ilişki kolonunun genişliğine göre kırpılır (`internal/render/diplom.go`).
...
Kara bölge paneli de deniz paneliyle aynı komşu başlığını (`Komşu Bölgeler`) içerir; aktif olaylar ve komşu listesi bina grid'i ile aksiyon barının altındaki panel-local scroll viewport'unda her oyun modunda gösterilir, deniz komşuları mavi tonla ayrıştırılır. Bölge aksiyon düğmesinin çizim ve hit-test başlangıcı aynı üretim/stat yerleşim hesabından türetilir; böylece `Diplomasi` düğmesi paneldeki gerçek konumuyla tıklanabilir kalır (`internal/render/panel.go`).
