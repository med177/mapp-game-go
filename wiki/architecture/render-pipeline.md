...
| 10 | Savaş ilan, savaş planı, kuşatma kararı, savaş raporu, genel onay, zafer detay ve event detail diyalogları; savaş ilan modalı artık tek satırlık `evet/hayır` onayı değil, iki cepheli koalisyon önizlemesi çizer. Sol tarafta oyuncu cephesi, sağ tarafta savunan cephe görünür; kesin katılan vassallar ve mevcut savaş içindeki müttefikler ayrı kartlarda listelenir, oyuncu kendi müttefikleri için checkbox ile çağrı seçer. Bu seçim `InputAction.WarAllies` alanıyla oyun katmanına taşınır. İlan resolve olduktan hemen sonra ayrıca ayrı `Savaş Özeti` modalı açılır; burada savaşa gerçekten katılan taraflar, katılmayan müttefikler ve iki cephenin toplam askeri gücü gösterilir. Eğer aynı frame'de savaş raporu da üretilecekse battle report bu modal kapanana kadar kuyruğa alınır. Savaş planı modalı üç duruş kartı (`Agresif/Dengeli/Savunmacı`) ve her kartın altında aynı combat matematiğinden türetilmiş zafer şansı + tahmini kayıp özetini gösterir. Modal artık bağlama göre `Kara Muharebesi`, `Deniz Muharebesi` veya `Çıkarma Muharebesi` başlığıyla açılır. Resolve tamamlanınca oyuncu tarafında ayrı `... Raporu` modalı açılır; burada sonuç, duruş, resim/ses hook'u ve saldıran-savunan için `Güç`, `Birim`, `HP` değerlerinin `önce -> sonra` kırılımı görünür. Resim için `assets/ui/battle_<scene>.png`, ses için `assets/sounds/battle_<scene>.wav` isimleri beklenir; dosya yoksa generic fallback ve placeholder metin kullanılır. Oyuncu savaşta bir devletin son kara toprağını düşürürse battle report kapandıktan hemen sonra kuyruklanan `Savaş Sonrası Düzen` modalı açılır; burada zorunlu `İlhak Et / Vassal Yap` seçimi yapılır ve modal battle report bitmeden input almaya çalışmaz. Tahkimli kara hedefte ayrı `Kuşatma Kararı` modalı açılır ve `Kuşatma Başlat / Genel Hücum / Kuşatmayı Kaldır` akışı seçilir; modal ayrıca ordunun mevcut gedik kapasitesini `Tn/Tf` formatında gösterir. Kuşatma aktifken saldıran ordu render'da hedef bölgenin üstünde görünür ve bölge kuşatan renginin taralı overlay'iyle muallak gösterilir. Seçili kuşatma ordusu için modal-dışı alt-orta `Kuşatma Emri` paneli çizilir; oyuncu buradan `Genel Hücum` veya `Kuşatmayı Kaldır` seçebilir ve aynı anda normal hareket highlight'ları açık kalır. Kayıp satırları artık `HP` ve `Birim` olarak ayrılır; böylece henüz ölü üretmeyen ama ciddi hasar alan senaryolar da görünür olur. Zafer HUD kartına tıklanınca açılan modal aktif hedefin başlık, açıklama ve ilerleme özetini gösterir; victory detail modal içeriği sabit yükseklikte değil, viewport + wheel scroll + scrollbar ile aşağı akar; event detail popup structured historical log detayını da çok satırlı gösterir | `renderer.go`, `panel.go`, `cursor.go`, `ui_modals.go`, `battle_report.go`, `war_summary.go` |
...
Kara bölgelerde görünen yerleşim işaretleri `regions.json` içindeki `settlements[]` alanından gelir. `WorldMap` her yerleşim için `SettlementAnchor` hesaplar; koordinat yanlışlıkla bölge dışına verilirse log uyarısı basılır ve aynı region içindeki en yakın piksele fallback yapılır. `port` settlement'lar liman simgesi, `fortress` settlement'lar kale simgesi, diğerleri nokta olarak çizilir; ulusal başkent olan settlement'lara bunların yanında ek bir yıldız rozeti çizilir. Kara ordu ikonları mümkünse `port` olmayan yerleşim anchor'ına kaydırılır; dock edilmiş filolar ise `DockedSettlementID` ile doğrudan liman anchor'ında görünür. Nakliye filosu cargo taşıyorsa yuvarlak filo ikonunun üstüne küçük kare bir badge ve içindeki taşınan birlik sayısı çizilir.
...
10. Yerleşim seçimi (`settlementHitAt`)
11. Bölge seçimi (WorldMap pixel lookup)
...
Bina ve birim kartlarında hover tooltip vardır. Tooltip maliyet, gereksinim, temel etki/istatistik ve kart görselini gösterir. Bölgeye uygun olmayan bina kartları render edilmez; liman son sıradadır ve kıyı olmayan bölgelerde görünmez. Ordu detay paneli dost toprakta toparlanan hasarlı birimler için başlıkta kısa durum metni ve kart sağ üstünde küçük `+` rozeti gösterir.
