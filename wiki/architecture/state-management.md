...
snapshot tutar. Her fraksiyon için vergi/ticaret/rota/haracı ayrıştırılmış altın
geliri, sabit ordu `GoldUpkeep`, ödenen/ödenemeyen bakım ve tur sonu hazine burada
raporlanır; kalıcı hazine değeri yine `Faction.Gold` alanıdır. Böylece save
formatına geçmiş tur raporu eklenmeden HUD, gelir popup'ı ve oyuncu uyarıları
aynı çözüm sonucu kullanır. Tur içi hediye transferleri `GoldTurnLedger` runtime
alanında ayrıca tutulur.
...
kaybedilen birliklerin toplamı ile kara ordusu ve filo ayrımı, ele geçirilen bölge sayıları,
son muharebe turu ve son barış teklifi turu. `CasualtiesA/B` toplam alanları barış
değerlendirmeleriyle geriye dönük uyumluluk için korunur; `CasualtiesArmyA/B` ve
`CasualtiesFleetA/B` yeni muharebelerde ordunun türüne göre doldurulur. `FactionA`/`FactionB` sayaçların relation anahtarıyla uyumlu alfabetik taraflarını
...
Merchant filo görevi `Army.TradeRouteKey` ile kalıcı tutulur. `MerchantTradeRoutesForFleet()` yalnız filonun sahibi `FromFactionID` olan, yani kendi ihracatını temsil eden, aktif ve geçerli ticaret merkezi denizine sahip rotaları döndürür; karşı tarafın ihracat rotası merchant görevine atanamaz. `MerchantTradeRouteAssignedMerchantShips()` ve `MerchantTradeRouteHasCapacityForFleet()` aynı yönlü rotadaki tüm atanmış merchant gemilerini sayar; kapasitesi dolu rotaya yeni filo atanamaz, mevcut filo kendi aktif atamasını korur. `MerchantFleetTradeRouteCapacityBonus()` seçili filonun kapasiteden alabileceği `verilen/kalan` payı, `MerchantFleetTradeRouteBonus()` ise buna konum uygunluğu kapısını ekler. `SetMerchantTradeRoute()` oyuncu UI'sından gelen atamayı aynı state doğrulamasından geçirir. Rota anahtarı save/load ile korunur, merchant hacim bonusu ise ekonomi tick'inde gerçek filo konumu, görev ve ticaret merkezi kaynaklı kapasiteden yeniden türetilir. `MerchantTradeRouteEffectiveAmount()` abluka sonrası gerçek hacmi, `MerchantTradeIncomeForRoute()` ise yalnız taşınan merchant kargosunun gemi kârını hesaplar; `MerchantTradeRouteSafetyPercent()` devriye/escort desteğini kâr güvenliğine bağlar. Bu değerler runtime türevidir, save alanı değildir (`internal/state/merchant_trade.go`).
