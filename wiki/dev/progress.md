...
| Diplomasi paneli modern akış | ✅ | Solda devlet seçimi + sağda teklif paneli; savaş/barış/ittifak/ticaret için kabul olasılığı (%) ve durum göstergesi bulunur; teklif sayfasında üst bilgi blokları padding'li, footer icon+metin hizası ortak button primitive'iyle tutarlı çizilir. Bölge panelindeki hızlı diplomasi ve tam teklif paneli aynı validasyon helper'ını kullandığı için barış/savaş önkoşulu tek yerden hesaplanır ve geçersiz teklif gönderimi UI seviyesinde bloklanır; ticaret yüzdesi de artık gerçek kabul motoruyla aynı helper'dan beslendiği için panelde yüksek görünen teklif submit anında gizli bir sert trade kuralına takılmaz |
...
| Açılış kamera zoom ayarı | ✅ | İlk frame `resetCamera()` minimum sığdırma yerine `1.40x` yakın başlar; kampanya daha yakın açılır, sıkışık yerleşim kümeleri daha rahat ayrılır ve fare tekerleğiyle `4.5x` seviyesine kadar daha derin yakınlaşma yapılabilir |
