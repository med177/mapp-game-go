...
func TestApplyEconomyTickCombinesSatisfactionModifiers(t *testing.T) {
	...
	// Bölge 1: bina +2 + uzak savaş -1 + genişleme -1 + ordu gücü 100 => +10.
	if got := gs.Regions["region-1"].Satisfaction; got != 60 {
		t.Fatalf("toplam memnuniyet deltası bölge 1 için +10 olmalıydı, got=%d", got)
	}
	// Diğer bölgeler yalnızca uzak savaş yorgunluğu ve 20+ bölge cezasını alır.
	if got := gs.Regions["region-2"].Satisfaction; got != 48 {
		t.Fatalf("uzak savaş ve genişleme cezaları toplam -2 olmalıydı, got=%d", got)
	}
}
...
func TestApplyEconomyTickWarFatigueCountsIndependentRealmsOnly(t *testing.T) {
	...
	if got, want := fatigue["player"], 2; got != want {
		t.Fatalf("iki uzak bağımsız düşman realm için oyuncu cezası %d olmalıydı, got=%d", want, got)
	}
	if got, want := fatigue["enemy-a"], 1; got != want {
		t.Fatalf("enemy-a tek bağımsız karşı realm için %d almalıydı, got=%d", want, got)
	}
	if got, want := fatigue["enemy-a-vassal"], 1; got != want {
		t.Fatalf("enemy-a vassalı overlorduyla aynı realm cezasını almalıydı, uzak savaş cezası -1 olmalıydı, got=%d", got)
	}

	applyEconomyTick(gs)
	if got := gs.Regions["player-region"].Satisfaction; got != 48 {
		t.Fatalf("oyuncunun iki uzak bağımsız düşmana karşı memnuniyeti 48 olmalıydı, got=%d", got)
	}
	if got := gs.Regions["vassal-region"].Satisfaction; got != 49 {
		t.Fatalf("vassal savaşı ayrı devlet sayılmamalı, uzak savaş cezası -1 olmalıydı, got=%d", got)
	}
}
