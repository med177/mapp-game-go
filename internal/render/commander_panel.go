...
func commanderPortrait(commander *army.Commander) *ebiten.Image {
	if commander == nil || commander.PortraitAsset == "" || ActiveScenarioPath == "" {
		return nil
	}
	path := filepath.Join(ActiveScenarioPath, "sprites", commander.PortraitAsset)
...
func drawCommanderPortrait(screen *ebiten.Image, commander *army.Commander, x, y, w, h float64) {
	...
	if img := commanderPortrait(commander); img != nil {
		bounds := img.Bounds()
		...
		DrawTextCentered(screen, "PORTRE", x+w/2, y+h/2-8, FaceSmall, ColorGray)
		DrawTextCentered(screen, "HAZIR DEĞİL", x+w/2, y+h/2+10, FaceSmall, ColorGray)
	}
...
func (r *Renderer) DrawCommanderPanel(screen *ebiten.Image) {
	...
	if len(available) == 0 {
		...
		for i, commander := range available {
			...
			drawCommanderPortrait(screen, commander, row.X+7, row.Y+5, 56, 56)
			textX := row.X + 72
			DrawText(screen, commander.Name, textX, row.Y+10, FaceSmall, ColorWhite)
			DrawText(screen, fmt.Sprintf("Seviye %d  |  %d XP", commander.Level, commander.Experience), textX, row.Y+30, FaceSmall, ColorGray)
			DrawText(screen, "Özellik: "+commanderTraitsSummary(commander), textX, row.Y+49, FaceSmall, ColorGold)
		}
	}
...
func drawCommanderProfile(screen *ebiten.Image, x, y float64, role string, commander *army.Commander) float64 {
	...
	DrawText(screen, fmt.Sprintf("Saldırı +%.0f%%  |  Savunma +%.0f%%", commander.AttackModifier()*100, commander.DefenseModifier()*100), x, y+62, FaceSmall, color.RGBA{140, 210, 150, 255})
	traits := make([]string, 0, len(commander.Traits))
	for _, trait := range commander.Traits {
		traits = append(traits, army.TraitLabelTR(trait))
	}
	traitText := "Henüz özellik kazanmadı."
	if len(traits) > 0 {
		traitText = strings.Join(traits, ", ")
	}
	DrawText(screen, "Özellik: "+traitText, x, y+82, FaceSmall, ColorWhite)
	return y + 112
}
