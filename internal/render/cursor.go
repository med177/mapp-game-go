...
func (r *Renderer) inGameHovering(fx, fy float64) bool {
	if topDateHudMenuButtonHit(fx, fy) || bottomActionButtonHit(fx, fy) || (imperialPanelAvailable(r.gs) && imperialHUDButtonHit(fx, fy)) || musicHudInteractiveHit(fx, fy) || activeWarsHudButtonHit(fx, fy) || turnTechHudTechHit(fx, fy) || turnTechHudTradeRouteHit(r.gs, fx, fy) || turnTechHudWarFatigueHit(r.gs, fx, fy) {
		return true
	}
