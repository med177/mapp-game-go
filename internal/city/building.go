...
type Building struct {
	...
	UpgradeCostMultiplier float64 `json:"upgrade_cost_multiplier"` // seviye maliyetinin bileşik artış çarpanı
...
func LoadBuildingsWithOrder(path string) (map[string]*Building, []string, error) {
	...
	for _, b := range list {
		...
		if b.UpgradeCostMultiplier < 1.0 {
			b.UpgradeCostMultiplier = 1.0
		}
